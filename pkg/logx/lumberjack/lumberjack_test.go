...
func TestDefaultFilename(t *testing.T) {
	currentTime = fakeTime
	dir := os.TempDir()
	filename := filepath.Join(dir, filepath.Base(os.Args[0])+"-lumberjack.logx")
...
func TestMaxBackups(t *testing.T) {
	...
	// Make a directory that exactly matches our logx file filters... it still
	...
	// Create a logx file that is/was being compressed - this should
	// not be counted since both the compressed and the uncompressed
	// logx files still exist.
	...
	// We should have four things in the directory now - the 2 logx files, the
	// not logx file, and the directory
	fileCount(dir, 5, t)
...
func TestCleanupExistingBackups(t *testing.T) {
	...
	// now create a primary logx file with some data
	filename := logFile(dir)
	err = ioutil.WriteFile(filename, data, 0644)
	isNil(err, t)
...
func TestMaxAge(t *testing.T) {
	...
	// We should still have 2 logx files, since the most recent backup was just
	// created.
	fileCount(dir, 2, t)
	...
	// We should have 2 logx files - the main logx file, and the most recent
...
func TestTimeFromName(t *testing.T) {
	l := &Logger{Filename: "/var/logx/myfoo/foo.logx"}
	...
	tests := []struct {
		...
		{"foo-2014-05-04T14-44-33.555.logx", time.Date(2014, 5, 4, 14, 44, 33, 555000000, time.UTC), false},
		{"foo-2014-05-04T14-44-33.555", time.Time{}, true},
		{"2014-05-04T14-44-33.555.logx", time.Time{}, true},
		{"foo.logx", time.Time{}, true},
	}
...
func TestCompressOnRotate(t *testing.T) {
	...
	// a compressed version of the logx file should now exist and the original
...
func TestCompressOnResume(t *testing.T) {
	...
	// the logx file should now exist and the original should have been removed.
	bc := new(bytes.Buffer)
	gz := gzip.NewWriter(bc)
	_, err = gz.Write(b)
	isNil(err, t)
...
// logFile returns the logx file name in the given directory for the current fake
// time.
func logFile(dir string) string {
	return filepath.Join(dir, "foobar.logx")
}

func backupFile(dir string) string {
	return filepath.Join(dir, "foobar-"+fakeTime().UTC().Format(backupTimeFormat)+".logx")
}

func backupFileLocal(dir string) string {
	return filepath.Join(dir, "foobar-"+fakeTime().Format(backupTimeFormat)+".logx")
}

// logFileLocal returns the logx file name in the given directory for the current
